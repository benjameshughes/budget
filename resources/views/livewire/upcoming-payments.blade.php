<div class="space-y-4">
    <div class="flex items-center justify-between">
        <flux:heading size="md">Upcoming Payments (30 days)</flux:heading>
        ...
        <flux:table.rows>
            @forelse($upcoming as $bill)
                <flux:table.row :key="$bill->id">
                    <flux:table.cell>{{ $bill->next_due_date->format('Y-m-d') }}</flux:table.cell>
                    <flux:table.cell>{{ $bill->name }}</flux:table.cell>
                    <flux:table.cell>£ {{ number_format($bill->amount, 2) }}</flux:table.cell>
                    <flux:table.cell>
