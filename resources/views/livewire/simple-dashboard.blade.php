<div>
    {{-- Account Health --}}
    <div x-data="{ healthOpen: false }" class="rounded-xl bg-white dark:bg-zinc-900 ring-1 ring-zinc-200 dark:ring-zinc-800 overflow-hidden mb-4">
        <button @click="healthOpen = !healthOpen" class="w-full flex items-center justify-between px-5 py-3">
            <span class="text-sm font-medium text-zinc-700 dark:text-zinc-300">Account Health</span>
            <span class="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold {{ $this->healthBadgeClasses() }}">
                {{ $this->accountHealth['label'] }}
                <flux:icon name="chevron-down" variant="micro" class="w-3 h-3 transition-transform" ::class="healthOpen && 'rotate-180'" />
            </span>
        </button>

        <div x-show="healthOpen" x-collapse class="border-t border-zinc-100 dark:border-zinc-800">
            <div class="divide-y divide-zinc-100 dark:divide-zinc-800">
                @foreach($this->accountHealth['signals'] as $signal)
                    @if($signal['status'] === 'grey' && $signal['link'])
                        <a href="{{ $signal['link'] }}" class="flex items-center justify-between px-5 py-3 hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                            <div class="flex items-center gap-2">
                                <div class="w-2 h-2 rounded-full {{ $this->signalDotClass($signal['status']) }}"></div>
                    ...
                    @elseif(isset($signal['progress']))
                        <div class="px-5 py-3">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <span class="text-sm font-medium {{ $this->signalValueClass($signal['status']) }}">{{ $signal['value'] }}</span>
                            </div>
